...
- general requirements for the drivers:
  1. be ahead of time(15/20 minutes).
  1. communication between a driver and a client should be simple(let the client know when he is on location).
  1. besides communication, driver opens the door, carries the luggage, helps the client to feel comfortable(newspapers/magazines, bottled water, optimal temperature, etc.), no colones or air fresheners.
  1. drivers are not allowed to enter the house.
  1. driver shouldn't engage in conversation, focused on driving, familiar with the area.
  1. driver according to the traffic rules.
  1. driver takes care of the car(washes the car, refuels, etc.)
  1. drivers should listen to clients when choosing routes.
  1. they are not allowed to take payments, but can accept tips.
  1. should let us know if he is going to be late.
  1. should let the office know if the driver left something.
  1. if the driver can't find location might call to ask.
...
- after finding the drive we want to farm out:
  1. set the FOT cost.
  1. the name of affiliate.
  1. select button farm-out.
  1. if affiliate uses *Limoanywhere*:
    ...
    1. affiliate should accept the drive.
  1. if affiliate doesn't use *Limoanywhere*:
    ...
    1. Send a message asking them if they will accept the job.
...
- How to create an account?
  - there are two options:
    1. Through already existing reservation:
        - after entering the `Edit Reservation` page, check-in `Copy from here` and `Create and account`
        - the system will use the information(account info and financial data) from the reservation.
        - press `Save` and the account has been created.
    1. Creating new account without already existing reservation:
      - go to `Accounts` section of the main page
      - enter data manually
      - account type(usually all three options are true but chose options that apply)
      - `Internal private notes`(only for agents): important notes about the clients e.g. doesn't like music.
      - `Preferences` and `Notes for drivers`: drivers can see them, but it's a good idea to share these notes with drivers.
      - In `Financial Data` section is where we enter credit card info.
...
- How to charge a client?
  ...
  1. when checking the reservation we can see outstanding funds, that means that the payment hasn't been made.
  1. the next choose `Authorize and Capture` option.
...
- How to hold the funds?
  ...
  1. Press `Submit`.
  1. Then press `Capture`, to charge the remaining amount of funds.

- How to do the refund?
  1. in `Edit Reservation`
  1. Visit `Payment` page, 
  1. use the `Void` button to void the charge..
  - if the charge was already approved `Credit` button should be used so that clients funds are credited within seven days.
...
- Are *road show* and *as directed* subcategories of *hourly service*?
- What is the dynamic of work, since we need to research affiliates?
- Are we gonna use *limoanywhere* software?
- Is *point to point* same as *door to door*?
- The wait time is from 10-15 minutes from all door-to-door services except for airlines?
